...
//
//
// NULL         non-NULL    "Index": some entries present. (Erik: "Sparse", not
// compressed, coo)
//                          indices need not be in order, nor unique.
//                          size of index [k] array is nindex [k].
//                          in_order [k] can be true or false.
//
// NULL         non-NULL    "Hyper_ELLPACK:4", because p is a simple stride [0 4
// 8 ...]
//
...
//                                  any given row is empty, or has exactly 4
//                                  entries.
//
// non-NULL     non-NULL    "Hyper": some entries present. (Erik: "DC" "doubly
// compressed")
...
//
// non-NULL     NULL        "Sparse": all entries present. (Erik: "Compressed"
// or C)
...
The Sparse and Hyper formats have a pointer, so if the axis k is Sparse or
Hyper, the objects they describe to the right of them in axis k+1 can have
variable sizes (any format, but only Hyper has variable size).
...
//                                  pointer(0) = [0 N 2*N ... M*N M*N+1] of size
//                                  M+1. if the matrix is M-by-N-by-K.
